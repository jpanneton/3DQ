...
void AbstractRingBuffer::finishOperation(std::atomic<int>& cursor, int finalBlockSize) const noexcept
{
	auto newPos = cursor.load() + finalBlockSize;

	if (newPos >= getTotalSize())
	{
		newPos -= getTotalSize();
	}
...
AbstractRingBuffer::OperationResult AbstractRingBuffer::prepareToWrite(int numToWrite) const noexcept
{
	const auto head = m_head.load();
	const auto tail = m_tail.load();
	...
	return generateResult(tail, head, numToWrite);
}

void AbstractRingBuffer::finishedWrite(int numWritten) noexcept
{
	jassert(numWritten >= 0 && numWritten < getTotalSize());
	finishOperation(m_tail, numWritten);
}

AbstractRingBuffer::OperationResult AbstractRingBuffer::prepareToRead(int numToRead) const noexcept
{
	const auto head = m_head.load();
	const auto tail = m_tail.load();
	...
	return generateResult(head, tail, numToRead);
}

void AbstractRingBuffer::finishedRead(int numRead) noexcept
{
	jassert(numRead >= 0 && numRead <= getTotalSize());
	finishOperation(m_head, numRead);
}